...
结论：js加载/执行，会阻塞DOM树的解析和渲染
原因：js会涉及dom和css的操作

结论：js加载/执行，会阻塞js对DOM树的css操作
原因：css animation不受影响

结论：css加载/执行，不会阻塞DOM树的解析
原因：见webkit渲染过程中的cssom

结论：css加载/执行，不影响DOM树解析，但会阻塞DOM树渲染，如果外链css之前没有节点，则显示白屏
原因：浏览器优化机制

结论：css加载/执行，会阻塞js执行
原因：渲染使用GUI进程，js执行使用v8，两者互斥
