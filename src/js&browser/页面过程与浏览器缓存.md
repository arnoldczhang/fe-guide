...
3. （如果不是ip地址）DNS(domain name system)解析（浏览器缓存、本机缓存、hosts、路由缓存、ISP缓存、递归）
4. TCP3次握手
...
* <script defer>: 异步加载，延迟执行，加载不阻塞html-parse
* <script async>: 异步加载，加载不阻塞html-parse，执行会阻塞
