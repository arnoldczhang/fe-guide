...
| js 加载/执行 | **阻塞** js 对 DOM 树的 css 操作 | js 会涉及样式操作，但 css animation 不受影响 |
| css 加载/执行 | **不会阻塞** DOM 树的解析 | 见 webkit 渲染过程中的 [cssom](#CSSOM) |
| css 加载/执行 | **不会阻塞** DOM 树解析，但**会阻塞** DOM 树渲染，如果外链 css 之前没有节点，则显示白屏 | 浏览器自身优化机制 |
| css 加载/执行 | **阻塞** js 执行 | 渲染使用 GUI 进程，js 执行使用 v8，两者互斥 |
...
1. 输入 URL
2. 检查缓存（请求 200、强缓存 200、协商缓存 304）
3. DNS 解析
   - 浏览器缓存
   - 本机缓存
   - hosts
   ...
   - 递归（见下图）
4. TCP3 次握手
5. 请求，解析 url，设置请求报文（头、体）
6. 服务器返回 html，响应报文，4 次挥手
7. 浏览器渲染
...
- 检验新鲜通常有两个 HTTP 头进行控制 Expires 和 Cache-Control：
  ...
  * Cache-Control还支持stale-while-revalidate，表示超出新鲜时间后仍能使用缓存的期间，同时会在后台请求新资源
- 如果通过 Expires || Cache-Control，判断已过期，则与服务器进行验证
  * 通过服务端 Last-Modified && Etag 判断服务端是否更新，没更新也走缓存（协商缓存：304）
  * 缓存存放，分 from memory（内存）和 from disc（磁盘），**有 Etag 才表示存磁盘**

### 3. 解析 URL 获取协议，host，端口，path

### 4. 组装一个 HTTP（GET）请求报文

### 5. DNS(domain name system)解析

如果不是 ip 地址，需要获取主机 ip 地址，ip 地址找 mac 地址，检查过程如下：
...
- 根名称服务器(.)（[为什么只有13台根root](https://www.zhihu.com/question/22587247/answer/66417484)）
- 顶级名称服务器(.com)
...
- DNS报文被要求控制在`512字节`之内
- 每个根root都要在报文占一定字节，剩余字节要留给包装DNS报头和其他协议参数
- 13台是比较合适的数量

**真的只有13台？**

不是，13台只是个概念，背后有多台真正的物理服务器在工作（截止2020年8月12日，共1097台）
