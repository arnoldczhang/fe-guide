...
- [http相关知识谱](https://juejin.im/post/6857287743966281736?utm_source=gold_browser_extensions)
...
12. 浏览器接收HTTP响应，然后根据情况选择关闭TCP连接或者保留重用，关闭TCP连接的`四次挥手`如下：
  ...
  - 主动方发送ACK=Y， Seq=X报文
13. 浏览器检查响应状态吗：是否为1XX，3XX， 4XX， 5XX，这些情况处理与2XX不同，比如3XX，有个重定向计数器，超过次数也会报错
14. 如果资源可缓存，进行缓存
15. 对响应进行解码（例如gzip压缩）
...
## dns
> DNS域名系统，是应用层协议，运行`UDP`协议之上，使用端口**53**
...
### 查询协议
> udp，因为 udp 连接最少可以只有两个包，而 tcp 至少9个包（三次握手，一次查询包，一次响应包，四次挥手）
...
* etag适合体积小的资源，express适合不重要但体积大的资源
...
* 低效，量大
* 判断HTTP缓存头
* 缓存大文件（比如大的js、css）或系统内存使用率高时，优先Disk Cache
