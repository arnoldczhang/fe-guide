...
| ------- | ------------- | ------------ | ------------ | -------------- |
| 协议层     | tcp           | tcp          | tcp          | udp            |
| 连接方式    | 默认短连接         | 默认长连接        | 多路复用         | quic多路复用       |
| 头部压缩    | /             | /            | HPACK        | QPACK（标头解码不阻塞） |
| 队头阻塞    | /             | 应用层的阻塞       | 传输层的阻塞（丢包重传） | 解决             |
| 服务端推送   | /             | /            | 支持           | 支持（优化）         |
| 握手延迟    | 高             | 中            | 中            | 低（0rtt）        |
| 场景      | 静态网页          | 现代网站         | 高性能网站        | 移动网络           |
| 1个tcp连接 | 发起&处理一个http请求 | 串行处理多个http请求 | 并行处理多个http请求 | 并行处理多个http请求   |
...
> - http版本是**应用层**协议
> 
> - http和https是**传输层**加密方案
> 
> - 技术上，**支持http2.0用在http**，但浏览器一般会禁止此项
...
- 对**同一域名**，http1.1支持最多并发 6 个tcp连接，h2 没有限制，不同的浏览器有一些区别
...
sourceBuffer：拼接视频资源片段（参考[B站视频range](https://mp.weixin.qq.com/s?__biz=Mzg3OTYzMDkzMg==&mid=2247497087&idx=1&sn=0d7daa4c11be8f8535a623e388b59dde&scene=21#wechat_redirect)）
...
* 浏览器对同域名，有**6-8个并发tcp连接**的限制，所以要拆分多个cdn地址
...
* 减轻服务端负载
* 不用再因为浏览器针对同域名的6-8个并发tcp连接限制，而拆分多个cdn地址了
* ![多路复用](多路复用.png)
...
* prefetch
* **chrome106版本已移除此功能**，因为使用率低，收益不明显，会通过状态码：103（加载提示）替代
...
- 可靠性：包编号、流复用
- 效率：0RTT，可插拔式拥塞控制（针对不同网络状况，使用不同算法优化传输速度）
- 容错性：前向纠错（数据包嵌入少量冗余数据，少量丢包原地修复，减少重试次数，提升弱网稳定性）
- 自适应超时（根据网络延迟，动态调整超时重传时间）
- 安全性：内联加密（深度集成TLS1.3）
