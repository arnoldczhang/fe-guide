...
- 307：和302的区别是，不会把部分 POST 转为 GET
...
* 三次握手需要1.5个rtt
* 建立tls（根据tls版本不同，1.2或1.3），需要1-2个rtt

**队头阻塞**

> http1.x出现丢包时，tcp会启用丢包重传机制，由于并行请求跑在多个tcp管道，只会影响当前管道的后续请求，其他管道无影响
>
> http2出现丢包后，同样会重传，不过由于所有请求复用了同一管道，所以阻塞后续请求的影响比1.x更大

---

## quic
> 基于UDP，0rtt建连，汲取了tcp的精华
