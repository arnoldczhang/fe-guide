...
**模板组件**: 侧重可扩展性和包容性，状态纯依赖外部；再进一步，可拆分为UI + 通用逻辑两层

### 注释规范

1. 用 /** */ 形式写注释，以便编辑器识别后，作为hover提示
2. description、version、deprecated、default

```jsx
export interface MyInputsProps {
  /**  custom class */
  className?: string
  /**
...
1. 和UI有关的状态变化，考虑预留emit事件

```jsx
const MyInput = (props: MyInputProps) => {
  const { onOpen, onCollepse } = props;
...
```tsx
const MyShowPage = (props: MyTableProps, ref: React.LegacyRef<Table>) => {
