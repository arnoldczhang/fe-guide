...
- 异步组件：defineAsyncComponent（组件默认就会是按需加载）
```js
const ComponentA = defineAsyncComponent(() =>
  /**
   * vite和webpack 都会识别这里的 import()
   * 1. webpackChunkName是个魔法注释，webpack会读取作为chunkFilename
   * 2. vite可以通过manualChunks(id)，修改chunkFilename
   * */ 
  import(/* webpackChunkName: "chunk-a" */ './components/ComponentA.vue')
);
```
...
| [FMP](https://mp.weixin.qq.com/s/XZZMYPX12PxDXtojeDk7MA) | 首次有效渲染（已弃用）           | 太主观，用`LCP`替代      |
| LCP | 最大内容元素渲染              | 核心指标              |
